...
subprocess.run([str(Path(a.java_home)/'bin/javac'),'--release','25','-proc:none','-cp',cp,'-d',str(classes),*map(str,root.rglob('*.java')),*map(str,(root.parent/'concurrent-server-core/src').rglob('*.java'))],check=True)
jar=out/'lss-rig-fabric-concurrent.jar'
