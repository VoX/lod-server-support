...
`AntiXrayCompat` (Fabric, docs/planning/antixray-compat-design.md) is the DrexHD AntiXray accommodation, same zero-compile-dep pattern: (1) the crash shim — binds AntiXray's obfuscation-context ScopedValues to null around both serialize choke points (their mixins' raw `.get()` throws unbound otherwise; the probe path additionally has generic containment in `RequestProcessingService.serializeProbeContained`), and (2) the engine probe — per-world adoption of the mod's hidden list + max-block-height off its controller for the x-ray masking feature (`XrayMaskPolicy` in common, `XrayMaskFilter`/`XrayMaskManager` + Paper twins; masking hooks INSIDE `serializeColumn`/the NBT serializers so every serve path and the DirtyContentFilter hash see identical masked bytes; the static manager holder is published by the service lifecycle with an owner-guarded retract). All failure shapes degrade fail-safe (inactive shim = contained blank LODs, unreadable engine = LSS-config-keys masking) with once-bounded logging.
...
- `lss-server-config.json` — bandwidth caps, LOD distance, disk reader threads, read priority, send queue limits, generation toggle, dirty broadcast interval, generation concurrency caps (server-internal; the old `syncOnLoadConcurrencyLimitPerPlayer` knob is retired — a constant now, and the key is dropped from the file on the next save), legacy-client shim toggle (`enableV16Compat`, default true), miss-memo TTL (`missMemoTtlSeconds`, default 30, 0 = off), x-ray LOD masking (`xrayObfuscation` "auto"/"on"/"off" default auto — auto detects the AntiXray mod on Fabric / Paper's built-in anti-xray per world and adopts the engine's hidden list + max height; `xrayHiddenBlocks` + `xrayMaxBlockHeight` (default 64) are the fallback tier — docs/planning/antixray-compat-design.md)
- `lss-client-config.json` — receive toggle (`receiveServerLods`), distance override (`lodDistanceChunks`), legacy-server backward-compat (`enableV16ServerCompat`, default true) + its Tier B generation-drive (`enableV16Generation`, default true — see `docs/planning/v16-client-compat-design.md`)
