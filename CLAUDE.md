...
### Networking Protocol (v17 — declarative want-set)
...
**C2S:** `HandshakeC2SPayload` (capabilities bitmask) → `BatchChunkRequestC2SPayload` (batch of packed position + clientTimestamp pairs — under v17 this is the client's **complete want-set**, not an increment)
...
- **`RESPONSE_UP_TO_DATE` / `RESPONSE_NOT_GENERATED` are the only things that empty the want-set.** They are what makes `ColumnStateMap.classify` return SATISFIED. Never delete them.
...
- **The want-set cap must dominate the per-player slot caps**, or the window never exceeds what is already in flight and frontier discovery starves. Enforced by `ServerConfigBase.validate()`'s cross-clamp (Global Constraint #28): after the per-field clamps it caps `syncOnLoadConcurrencyLimitPerPlayer` then `generationConcurrencyLimitPerPlayer` so that `syncCap + genCap*SCAN_BUDGET_MULTIPLIER + WANT_SET_FRONTIER_RESERVE <= MAX_BATCH_CHUNK_REQUESTS` always holds — sync (primary discovery) gets a fixed ceiling (956), gen gets the remaining budget. `SpiralScanner.BUDGET_MULTIPLIER` reads `LSSConstants.SCAN_BUDGET_MULTIPLIER` so the scan-budget derivation and the clamp share one source of truth. Pinned by `maxConfigLeavesFrontierHeadroomInTheWantSetBudget` (Fabric + Paper). If you change `MAX_CONCURRENCY_LIMIT`, `MAX_BATCH_CHUNK_REQUESTS`, or the multiplier, those tests re-derive the boundary automatically.

Backpressure lives in three places now, none of them on the wire: the bounded want-set (`MAX_BATCH_CHUNK_REQUESTS` = 1024; the scan budget is 4× the sync slot cap, clamped to it), the server's per-player dequeue gates (a full slot cap or disk pool *retains the entry in the backlog* — it never bounces), and the client's edge-triggered clear above.
...
Flow: client handshakes with capabilities bitmask (CAPABILITY_VOXEL_COLUMNS set if LSSApi has consumers) → server sends session config with concurrency limits → client scans in an expanding spiral every second (20 ticks) and ships its whole want-set with clientTimestamps (-1 for unknown/first request, 0 for generation request, >0 for resync) → the batch lands in the player's latest-wins mailbox and, on the processing cycle, **replaces** the backlog (superseded entries counted, out-of-range ones range-filtered) → the router drains the backlog in declaration order (proximity-ordered by the client): up-to-date checks, loaded-chunk probe, then for a cold column `tryAdmitAndSubmit` — sync path (ts > 0 or -1) takes a sync slot and reads disk, generation path (ts == 0) takes a slot and generates; if the slot cap is full the entry is **retained and the drain continues** (so a full sync cap never starves generation behind it), and if the disk pool has no headroom it is retained and the pass **stops** → server batches lightweight responses (up-to-date, not-generated) per tick; column data payloads are sent individually. After the initial scan, the server periodically pushes `DirtyColumnsS2CPayload` listing changed columns; a dirty position re-enters the client's want-set on the next scan.
...
- `IncomingRequestRouter` — replaces each player's backlog with the newest declared want-set, then drains it in declaration (proximity) order: checks timestamps for up-to-date, probes loaded chunks, dispatches disk reads (registering the generation in-flight guard on the direct-generation path), with cross-player dedup via `DedupTracker`. Nothing is bounced: a full slot cap **retains the entry and keeps scanning** (the cap means "dequeue at most N concurrently", not "reject above N" — a full SYNC slot must not starve admissible GENERATION entries behind it), no disk headroom **retains and stops the pass** (saturation never reaches the wire — issue #32 at its source), and a full send queue retains and stops. Retained entries are re-prepended in order by `restoreBacklog`, which also republishes the want-set
...
- `SpiralScanner` — expanding Chebyshev ring scan (20-tick cadence) that produces the want-set closest-first; owns scan policy: request budget (4× the server's sync slot cap, so the window always has frontier headroom) with queue-pressure and vanilla-load scaling. Does NOT suppress awaited positions — an awaited position blocks its own ring's confirmation
- `ColumnStateMap` — single owner of per-column client state (timestamps + dirty/retry/validated marks); its `classify` ladder decides whether a position is still wanted, and marks are consumed at the terminal answer (up-to-date / not-generated), not at send
...
- `lss-server-config.json` — bandwidth caps, LOD distance, disk reader threads, read priority, send queue limits, generation toggle, dirty broadcast interval, concurrency limits
...
- `backpressure.superseded` / `range_filtered` — asks silently dropped by a newer want-set declaration / by the distance filter; healed by re-declaration. Nonzero is normal under v17 — `superseded` rises with any backlog the 1 Hz declare outruns
...
`rate-limit-storm` and `disk-saturation` keep their **historical names** but no longer test rate limiting — v17 has none. They now test the queue-absorption premises that replaced it, and **both premises are now measured live, not derived** (the first live run corrected one of them — see below):
...
- **`rate-limit-storm` asserts a *ceiling*, `service.superseded <= 50` (measured: 0)** — not a floor. **Under v17 a small slot cap cannot create contention**, because the client's scan budget IS `syncOnLoadConcurrencyLimitPerPlayer × BUDGET_MULTIPLIER` (`SpiralScanner`): this scenario's `syncCap: 4` caps the entire want-set at 16 positions/scan, so four slots serving sixteen wants per second is abundance. The v17 rewrite originally asserted `superseded >= 100` here on the assumption of "~800-entry want-sets" — that number silently assumed the *default* syncCap of 200 (200×4=800) and is unsatisfiable at 4. The ceiling now pins the want-set/gate coupling itself: decouple the budget from syncCap and supersession appears there immediately.

**The general rule this cost a live run to learn: under v17, supersession is produced by slow *service* relative to the want-set, never by a small slot cap** — shrinking the gate shrinks the want-set with it. A scenario that means to create want-set contention must slow the service (disk threads / generation), not the gate. A failure in either scenario is a premise question first (did the config stop creating contention? did the headroom gate leak?) and a threshold question only after.
...
- `soak-results/<scenario>-<timestamp>/server.jsonl` — server snapshot/command/join/end event rows. Snapshots carry the data-capture fields: `service.{superseded,range_filtered,re_resolved}`, `players[].backlog`, `*_hw` high-water gauges (per-tick sampled), `dedup.groups`, `jvm.*`, `tscache.*`, `dirty.{marked,suppressed}_total`, `mspt_avg_window`; command rows carry `ok`.
