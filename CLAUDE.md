...
- **The want-set cap must dominate the per-player slot caps**, or the window never exceeds what is already in flight and frontier discovery starves. This holds today only by constant choice — `MAX_BATCH_CHUNK_REQUESTS` (1024) > `MAX_CONCURRENCY_LIMIT` (1000) — with **no cross-check in `ServerConfigBase.validate()`** (`SpiralScanner`'s javadoc points there, but the check does not exist). Raising `MAX_CONCURRENCY_LIMIT` to/past 1024 would starve the frontier silently; add the guard if either constant ever moves.
