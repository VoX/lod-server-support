...
`rate-limit-storm` and `disk-saturation` keep their **historical names** but no longer test rate limiting — v17 has none. They now test the queue-absorption premises that replaced it: the storm asserts `service.superseded >= 100` (the slot gate leaves entries undrained and re-declaration heals them) and disk-saturation asserts `disk.saturated == 0` (the `hasHeadroom` gate must *prevent* what used to bounce — issue #32's root cause). Both floors are derived, not yet measured live: a failure there is a premise question first (did the config stop creating contention? did the headroom gate leak?) and a threshold question only after.
