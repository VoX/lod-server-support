...
class LibsDisguisesReloadReviewTest {
    ...
    private static byte[] bytes(String name) throws ClassNotFoundException {
        try (var in = LibsDisguisesReloadReviewTest.class.getClassLoader()
    ...
    private static class PluginLoader extends ClassLoader {
        PluginLoader() { super(LibsDisguisesReloadReviewTest.class.getClassLoader()); }
    ...
    private static class BridgeLoader extends ClassLoader {
        ...
        BridgeLoader(PluginLoader plugin) {
            super(LibsDisguisesReloadReviewTest.class.getClassLoader());
            this.plugin = plugin;
        }
        @Override protected synchronized Class<?> loadClass(String name, boolean resolve)
                throws ClassNotFoundException {
    ...
    @Test void changingPluginInstanceMustReadTheNewPluginsDisguiseRegistry() throws Exception {
        ...
        var currentPlugin = new AtomicReference<Object>(oldPlugin);
        ...
        currentPlugin.set(newPlugin); // production plugin-instance re-resolution trigger
