...
- **Seated far-player recovery on 1.21.1** — Restores passenger pose state after a failed vehicle render so later players in the same frame are drawn correctly.
...
- **Platforms** — Fabric, Paper/Purpur and NeoForge on Minecraft 1.21.1. Far-player rendering is available on both client loaders. This support line remains best-effort; Folia is unavailable.
