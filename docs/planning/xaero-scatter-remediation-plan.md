...
- **Ported to 1.21.1 (2026-09-05, PR against the line's support branch):** fabric-api 0.116.15 nests lifecycle-events 2.6.0 → the 2-arg `(level, chunk)` callback; `getPos().x` field spelling; NeoForge 21.1.248 posts `ChunkEvent.Load` from `ChunkStatusTasks.lambda$full$2` with a `ChunkAccess` (the seam's `instanceof LevelChunk` gate covers it); the owed-set hunks apply around the contract-pinned `getMinBuildHeight()/getMaxBuildHeight()` hunk; NeoForge gametest smoke 8/8; WI-6: the shared test sentence + both extractor comments only. Gates per the port PR. Smoke (2026-09-05) `cold-restart-resync` PASS: marked 0 / suppressed 441 / seeded_load 441, 2144 up_to_date, 0 columns re-sent.
