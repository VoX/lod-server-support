...
- **Ported to 26.1 (2026-09-05, PR against the line's support branch):** fabric-api 0.151.0+26.1.2 nests lifecycle-events 4.1.0 → the 3-arg `(level, chunk, generated)` callback (same as 26.2); everything else verbatim; WI-6 M1 (`LSSClientNetworking` comment: class AND member), the shared test sentence, banner, both extractor comments. Gates per the port PR.
