...
- **Gameplay always wins** — LOD disk reads now run below normal chunk loading on all platforms, and on Paper (and experimental Folia) LOD terrain generation runs at low priority too, so player-driven world loading takes precedence over LOD streaming.
