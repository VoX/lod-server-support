...
- **Tier B — drive generation (IMPLEMENTED, opt-in `enableV16Generation`, default off).** In v16
  ...
  old server generates on demand. Fills cold terrain. The "possibly re-generates disk-resident
  chunks" worry is RESOLVED against v0.6.2 from its source: its generate path (`ts==0`→`GENERATION`)
  is disk-FIRST when a disk reader exists (always, in production), so already-generated terrain is
  served and only true misses generate — same net effect as v18 server-owned generation. Isolated
  to the `sendRequests` wire copy — the core loop still emits `-1`, so `classify`/tracker/cache are
  untouched. Default off because (a) it drives generation load on a remote old server that never
  opted in, and (b) the disk-first guarantee is verified only for v0.6.2, not the whole
  v0.4.x–v0.6.2 range.
