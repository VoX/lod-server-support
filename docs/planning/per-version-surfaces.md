...
| 22 | Chunk-load seam (the dirty content filter's load baseline, xaero-scatter-remediation-plan.md WI-1b, ported 2026-09-05) | THIS line registers the 2-arg `ServerChunkEvents.Load.onChunkLoad(level, chunk)` of fabric-api's 2.x lifecycle module (0.116.15+1.21.1 → lifecycle-events 2.6.0); fabric-api 4.x (26.2) passes a third `generated` flag; NeoForge `ChunkEvent.Load` is line-invariant. Both fire from the FULL status task (this line's 2.6.0 `ChunkGeneratingMixin`; the NeoForge `ChunkStatusTasks` patch) — verified at port time (`javap` of the resolved lifecycle jar); Moonrise/C2ME fire it through their platform hooks (live instrument: `seeded_load=` in `/lsslod diag`) | The lambda literal in each loader's `LSSServerNetworking`; `DirtyContentFilterTest.seedLoaded*` + `ServiceLifecycleGameTests.pendingLoadSeedsFlushFromRealLoadedChunks` + `check_cold_restart_resync` (marked ≤ 50 on a restart) |
