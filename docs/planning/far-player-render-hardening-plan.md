...
Everything in §2 WI-1..WI-7/WI-10/WI-11 and every §10 row (1-8, 10-12) is ported to the two
26.x Fabric renderers; the NeoForge twins stay `RENDER_AVAILABLE = false` stubs (they gain
`diagLine()` for the xplat command coupling), the server-side privacy items (WI-7a/b) ship on
NeoForge through the xplat snapshot + `LSSNeoPermissions`. Per-line shapes that differ from
the 1.21.1 text (the reviewer's map):

| Item | 1.21.1 | 26.2 / 26.1 |
|---|---|---|
| WI-1 light | `packedLightFor` → `dispatcher.render(..., light)` | same helper on `LightCoordsUtil`; the value is written INTO the extracted state (`renderState.lightCoords = light`, `vState.lightCoords = ...`) before `dispatcher.submit` |
| WI-3 frustum | `context.frustum()` / `event.getFrustum()`; `noCulling`/`getBoundingBoxForCulling` | `context.levelState().cameraRenderState.cullFrustum` gated on `initialized`; `entity.getBoundingBox().inflate(0.5)` (the renderer members are protected here) |
| WI-4 batch end | `endLastBatch()` | n/a — the submit storage drains the frame; nothing to end (the arg-less `endBatch()` stays forbidden) |
| WI-5 byte owner | `Player.DATA_PLAYER_MODE_CUSTOMISATION` | `Avatar.DATA_PLAYER_MODE_CUSTOMISATION` (protected static, reached through the `RemotePlayer` subclass) |
| WI-6 tag draw | `Font.drawInBatch` ×2 | `SubmitNodeCollector.submitText` ×2 with OUR pose (translate anchor+0.5, `mulPose(cameraRenderState.orientation)`, scale) — background on the first, `POLYGON_OFFSET` on the second; the text phase draws in submission order |
| WI-6 (b) gap fill | literal 64 (`renderNames()`, `isSectionCompiled`) | 26.2: `Attributes.NAME_TAG_DISTANCE` per player (vanilla's cap is that attribute there — honoured both ways), hide-GUI = `gui.hud.isHidden()`, body gate = `isOutsideBuildHeight || levelRenderer.isSectionCompiledAndVisible` (vanilla's extraction gate verbatim), `xOld` lerp basis. 26.1: literal 64 (`EntityRenderer.extractRenderState`'s `4096.0`), `Minecraft.renderNames()`, the same section gate |
| §10 row 3 walk stop | `setSpeed(0) + update(0, 1)` | `WalkAnimationState.stop()` — verified to zero `speedOld` on both lines (the 1.21.1 sawtooth cannot occur); `WalkAnimationStopTest` not ported |
| §10 row 4/5 glide/swim | flag + `fallFlyTicks` + `AccessorLivingEntity` | same, plus `elytraAnimationState.tick()` per new tick — on 26.x the wing angles live in `ElytraAnimationState` (tick-advanced, reads the flag; a zero delta movement ramps to the full spread) where 1.21.1's `ElytraModel` derived them itself |
| §10 row 10 lift | `LiftedBufferSource`/`LiftingConsumer` — per-vertex radial pull on the buffer source handed to `dispatcher.render` | `LiftedSubmitCollector` — a `SubmitNodeCollector` wrapper handed to `dispatcher.submit` that re-submits every NON-skin submission (model / custom geometry / item / block model, `order(n)` sub-collectors wrapped too, Fabric's mesh-carrying FRAPI overloads forwarded with the mesh) with the captured pose PRE-MULTIPLIED by a uniform scale about the camera (`f = (L − lift·(1+0.8·tier))/L`, `L` = the proxy's camera distance): the projection is radial about the camera, so the screen position is unchanged and every vertex is pulled by `lift·r/L ≈ lift` along its view ray — the same screen-exact pull at pose granularity (the geometry is generated in the storage drain from the captured pose, not from a buffer source we hand in). Skin = the renderer's own model (identity) with its render type as the belt. Tiers from `EquipmentClientInfo` layers — `assets.get(assetId).getLayers(HUMANOID/HUMANOID_LEGGINGS)` → `layer.getTextureLocation(layerType)` → `RenderTypes.armorCutoutNoCull` (the `EquipmentLayerRenderer.layerTextureLookup` key with no player-texture override), + `Sheets.armorTrimsSheet(true/false)` + `RenderTypes.armorEntityGlint()`; the `EquipmentAssetManager` is a private final of the dispatcher, read through the new client accessor `AccessorEntityRenderDispatcher` (both mixin configs, pinned with the swim accessor) |
| §10 row 11 sentinel | `PoseStack.clear()` | `PoseStack.isEmpty()`; the stack pools its `Pose` objects so the mark's identity is stable |
| Mapping family | `ResourceLocation` | `Identifier` |
| 26.2 ↔ 26.1 | — | `mainCamera()` ↔ `getMainCamera()`; `gui.hud.isHidden()` ↔ `Minecraft.renderNames()`; the attribute ↔ literal-64 tag cap; the collector's abstract method set (26.1: `submitNameTag` 8-arg, abstract 11-arg `submitModelPart`, 2-arg `submitMovingBlock`, `submitBreakingBlockModel(BlockStateModel, long, int)`, `submitParticleGroup`, no `submitShapeOutline`/`submitGizmoPrimitives`/`submitCustom`, FRAPI without the breaking-block mesh overload) |
