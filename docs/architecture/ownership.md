...
On lines with Folia, a fresh declaration gets one pump tick for an owning-region probe. If that callback is late, disk fallback still proceeds. A valid disk-header or store freshness response, a successfully queued store body, and an eligible empty disk/store fallback preserve this opportunity too: persisted content does not establish that loaded content is unchanged. Empty fallback means either no-data up_to_date or a successfully queued authoritative clear; ghost, edit-stale, rejected-body and rejected-clear results cannot arm a correction. Each admitted disk attempt retains one bounded opportunity for a loaded probe, including positions beyond the initial 512-position probe window. New declarations, including empty backpressure batches, replace only unadmitted backlog; they do not cancel admitted reads or their corrections. The worker validates the captured result against its registration, dimension, edit epoch, range and exact disk-attempt identity. A corrective body waits until older pending reads and queued bodies for that column have drained, then consumes that exact retained attempt once. An all-air result explicitly clears terrain the client may already hold. Dirty events, retirement, range loss, expiry, rejected or excluded results, and replacement of the admitted attempt discard obsolete work. Retention is capped at 512 attempts and 2 MiB of serialized bodies per player for 10 seconds from the original admission; later declarations do not renew it, and an absent callback does not hold the request open. [Lifecycle](../../common/src/test/java/dev/vox/lss/common/processing/PairedIngressLifecycleTest.java) and [pump/router](../../paper/src/test/java/dev/vox/lss/paper/PaperPumpRouterAlignmentTest.java) tests cover these boundaries. Eight additional pump/router cases exercise store body/freshness and empty disk/store correction; four [freshness-rung exclusions](../../common/src/test/java/dev/vox/lss/common/processing/ReadFreshnessRungTest.java) reject ghost, stale and failed-enqueue paths.
