...
case "${1:-run}" in
    ...
    run-legacy)
        ...
        echo "   - Tier A (default): LODs render for terrain the server ALREADY generated, but"
        echo "     cold/never-visited terrain will NOT fill. Walk/fly around near spawn first to"
        echo "     generate terrain, THEN move out and watch the generated area appear as LODs."
        echo "   - Tier B (opt-in): set \"enableV16Generation\": true in the CLIENT's"
        echo "     config/lss-client-config.json and rejoin. Now flying into never-generated"
        echo "     terrain drives the old server to generate it on demand (cold terrain fills)."
