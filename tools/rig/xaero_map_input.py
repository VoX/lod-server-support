...
def move_away(driver,x,y):
 if type(x)is not int or type(y)is not int or not 0<=x<32768 or not 0<=y<32768:raise ValueError('invalid private map pointer action')
 driver.focus();driver.verify()
 if not driver.test.XTestFakeMotionEvent(driver.display,-1,x,y,0):raise ValueError('private map pointer move rejected')
 driver.x.XFlush(driver.display)
