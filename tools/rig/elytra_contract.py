...
def predicates(phase,run_id):
 ...
 return {
  'survival':('execute if entity '+'@a[name='+SUBJECT+',gamemode=survival] run say '+prefix+'SURVIVAL',prefix+'SURVIVAL'),
  'equipment':('execute if items entity '+SUBJECT+' armor.chest minecraft:elytra run say '+prefix+'EQUIPMENT',prefix+'EQUIPMENT'),
  'native_pose':('execute if entity '+'@a[name='+SUBJECT+',nbt={OnGround:'+('1b' if ground else '0b')+',FallFlying:'+('1b' if flying else '0b')+'}] run say '+prefix+'POSE',prefix+'POSE')}
def setup_commands():
...
def landing_command():return 'execute as '+OBSERVER+' at @s run tp @s ~ ~ ~ facing entity '+SUBJECT+' eyes'
