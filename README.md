...
| `lodDistanceChunks` | `512` | Default max LOD distance in chunks. Per-world overrides go in `lodDistanceChunksByWorld` |
| `lodDistanceChunksByWorld` | `{}` | Per-world LOD distance. Keys are Bukkit world names on Paper (`world_nether`, or a Multiverse world like `creative`) and dimension ids on Fabric/NeoForge (`minecraft:the_nether`). Paper also matches a dimension id as fallback. Unlisted worlds keep `lodDistanceChunks`. Change live with `/lsslod set lodDistanceChunks <world> <n>` (`<world> default` clears an override). Current-protocol clients switch to a world's distance the moment they enter it; a pre-0.10 legacy client picks it up when it (re)joins |
