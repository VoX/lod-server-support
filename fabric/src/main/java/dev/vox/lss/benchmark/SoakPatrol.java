 ...
 * swapped for a {@link ClientInput} whose {@code tick()} holds the forward impulse, and the
 * player's yaw is pointed at the current waypoint at the START of every client tick, so
 * vanilla's own aiStep/travel/sendPosition path does the walking and the packets. Re-applied
 * whenever the client player object changes (join, respawn, dimension change — vanilla installs
 * a fresh {@code KeyboardInput} there). A waypoint counts as reached within 1.5 blocks
 * horizontally; the walk then continues to the next one, wrapping around.
 *
 * <p>1.21.11 line: the driver is re-derived over this MC's {@code ClientInput} (1.21.2+
 * replaced the boolean-field {@code Input}/{@code KeyboardInput} pair): the key state is the
 * immutable {@code Input} record ({@code keyPresses}, what the movement packet carries) and the
 * impulse is the {@code moveVector} ({@code aiStep} reads it through {@code modifyInput}).
 */
final class SoakPatrol {
    ...
    private static final class PatrolInput extends ClientInput {
