...
public class CommandGameTests {
    ...
    public void soakScenarioGamerulesExecuteAndReadBack(GameTestHelper helper) throws Exception {
        ...
        Gt.assertTrue(helper, root != null, "could not locate actual scenario JSON files");
        ...
        Gt.assertTrue(helper, commandsToCheck.size() >= 5, "scenario setup command inventory must not be empty");
        ...
        for (String command : commandsToCheck) {
            ...
            Gt.assertTrue(helper, before.success(), "real rule query must succeed: " + query);
            try {
                Gt.assertTrue(helper, dev.vox.lss.benchmark.SoakCommandExecutor.setGamerule(commands, source, command),
                ...
                Gt.assertTrue(helper, dev.vox.lss.benchmark.SoakCommandExecutor.setGamerule(commands, source, query + " " + restore),
    ...
    public void soakCommandsRejectInvalidSyntaxAndMissingCallbacks(GameTestHelper helper) {
        ...
        for (String bad : java.util.List.of("gamerule lss_nonexistent_rule 0", "gamerule", "execute in")) {
            ...
            Gt.assertTrue(helper, rejected, "unknown/incomplete command must fail validation: " + bad);
        }
        ...
        Gt.assertTrue(helper, nested[0] != null && !nested[0].success(),
    ...
    public void soakCommandsDistinguishHandlerFailureAndWrongReadback(GameTestHelper helper) {
        ...
        Gt.assertTrue(helper, !dev.vox.lss.benchmark.SoakCommandExecutor.executeForResult(
                        commands, source, "lss_soak_test_failure").success(),
                "command failure callback must remain false");
        Gt.assertTrue(helper, dev.vox.lss.benchmark.SoakCommandExecutor.dispatch(
        ...
        Gt.assertTrue(helper, !dev.vox.lss.benchmark.SoakCommandExecutor.setGamerule(
